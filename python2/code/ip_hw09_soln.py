...
def spellcheck(d, legal):
# returns a new dictionary of all the words from d that are not in legal
    t = {}
    for word in d:
...
words = process_file('gatsby.txt')
print_most_common(words)

legal = process_file('/usr/share/dict/words')
print ''

illegal = spellcheck(words, legal)
print_most_common(illegal)
